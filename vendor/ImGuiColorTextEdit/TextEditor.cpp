...
void TextEditor::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
	...
	for (const auto& r : mLanguageDefinition->mTokenRegexStrings) {
		mRegexList.push_back(std::make_pair(boost::regex(
			r.first,
...
bool TextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
{
	...
	if (!mLocked)
	{
		...
		if (IsHorizontalScrollbarVisible()) foo.x -= window->ScrollbarSizes[0];
		if (IsVerticalScrollbarVisible()) foo.y -= window->ScrollbarSizes[1];
...
void TextEditor::MoveHome(bool aSelect)
{
	for (int c = 0; c <= mState.mCurrentCursor; c++) {
		...
		SetCursorPosition(Coordinates(mState.mCursors[c].mInteractiveEnd.mLine, newColumn), c, !aSelect);
	}
}
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	for (int c = mState.mCurrentCursor; c > -1; c--) // order important here for typing \n in the same line at the same time
	{
		...
		if (aChar == '\n')
		{
			...
			if (mAutoIndent) {
				for (int i = 0; i < cindex && i < line.size() && isascii(line[i].mChar) && isblank(line[i].mChar); ++i)
				{
			...
			AddGlyphsToLine(coord.mLine + 1, whitespaceSize, line.begin() + cindex, line.end());
			RemoveGlyphsFromLine(coord.mLine, cindex);

			SetCursorPosition(Coordinates(coord.mLine + 1, GetCharacterColumn(coord.mLine + 1, whitespaceSize)), c);
...
void TextEditor::TrimTrailingBlanksFromLine(int aLine) {
	if (!mTrimTrailingBlanks || mReadOnly || mLines.size() <= aLine || aLine < 0)
		return;
	/*
	...
	*/
	...
	while(lineEnd >= 0 && isascii(line[lineEnd].mChar) && isblank(line[lineEnd].mChar)) lineEnd--;
	if(++lineEnd < lineSize) RemoveGlyphsFromLine(aLine, lineEnd);
}
...
void TextEditor::HandleKeyboardInputs(bool aParentIsFocused)
{
	if (ImGui::IsWindowFocused() || aParentIsFocused)
	{
		...
		auto isOSX = false; // io.ConfigMacOSXBehaviors; // now seems to be handled by imgui itself
		auto alt = io.KeyAlt;
...
void TextEditor::Render(bool aParentIsFocused)
{
	...
	const float y_offset = 2.0; //(fontHeight * mLineSpacing - ImGui::GetTextLineHeightWithSpacing()); // TODO FIXME
	...
	if (!mLines.empty())
	{
		...
		for (int lineNo = mFirstVisibleLine; lineNo <= mLastVisibleLine && lineNo < mLines.size(); lineNo++)
		{
			...
			if (!haveSelection && cursorCoordsInThisLine.size() > 0)
			{
				...
				drawList->AddRectFilled(sstart, send, mPalette[(int)PaletteIndex::Selection] /* TODO */);
			}
			// Draw error markers
			...
			if (mShowLineNumbers)
			{
				...
				if (errorIt != mErrorMarkers.end())
				{
					...
					drawList->AddRectFilled(start, end, mPalette[(int)PaletteIndex::ErrorMarker]);
					drawList->AddText(ImVec2(lineStartScreenPos.x + mTextStart - lineNoWidth, lineStartScreenPos.y + y_offset), IM_COL32(255, 255, 255, 128), std::to_string(lineNo + 1).c_str());
					...
					drawList->AddText(ImVec2(lineStartScreenPos.x + mTextStart - lineNoWidth, lineStartScreenPos.y +y_offset), mPalette[(int)foreground], std::to_string(lineNo + 1).c_str());
				}
			}
			...
			while (charIndex < mLines[lineNo].size() && column <= mLastVisibleColumn)
			{
				...
				auto cx = lineStartScreenPos.x + mTextStart + TextDistanceToLineStart({lineNo, column}, false);
				...
				else
				{
					...
					if (0) {
						...
						ImVec2 end(start.x + mCharAdvance.x, start.y + mCharAdvance.y);
						drawList->AddRectFilled(start, end, IM_COL32(0, 0, 0, 128));
					}
					...
					if (1) {
						...
						const auto shadowColor = IM_COL32(0, 0, 0, 32);
						drawList->AddText(ImVec2(targetGlyphPos.x-1, targetGlyphPos.y), shadowColor, glyphBuffer.c_str()); // 32
						drawList->AddText(ImVec2(targetGlyphPos.x+1, targetGlyphPos.y), shadowColor, glyphBuffer.c_str()); // 32
						drawList->AddText(ImVec2(targetGlyphPos.x,   targetGlyphPos.y-1), shadowColor, glyphBuffer.c_str()); // 32
						drawList->AddText(ImVec2(targetGlyphPos.x,   targetGlyphPos.y+1), shadowColor, glyphBuffer.c_str()); // 32
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
	...
	for (int i = aFromLine; i < endLine; ++i)
	{
		...
		auto last = bufferEnd;
		bool bol = true;
		auto languageDefinition = (mSyntaxType[i] == 1) ? luaLanguageDefinition : asmLanguageDefinition;
		...
		for (auto first = bufferBegin; first != last; )
		{
			...
			{
				if (languageDefinition->mTokenize(first, last, token_begin, token_end, token_color))
			...
			if (hasTokenizeResult == false)
			{
				...
				for (const auto& p : mRegexList)
				{
					...
					auto flags = boost::regex_constants::match_continuous; // | boost::regex_constants::match_any;
					/*
					...
					*/
					...
					if (!bol) flags |= boost::regex_constants::match_not_bol;
					if (first > bufferBegin) flags |= boost::regex_constants::match_prev_avail;
					// printf("substr [%s] bow: %s\n", first, bow ? "true" : "false");

					try {
						regexSearchResult = boost::regex_search(first, last, results, p.first, flags);
					} catch (...) {}
			...
			else
			{
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					if (!languageDefinition->mCaseSensitive)
						std::transform(id.begin(), id.end(), id.begin(), ::toupper); // toupper

					if (!line[first - bufferBegin].mPreprocessor)
					{
						if (languageDefinition->mKeywords.count(id) != 0)
							token_color = PaletteIndex::Keyword;
						else if (languageDefinition->mIdentifiers.count(id) != 0)
							token_color = PaletteIndex::KnownIdentifier;
						else if (languageDefinition->mPreprocIdentifiers.count(id) != 0)
							token_color = PaletteIndex::PreprocIdentifier;
					}
					else
					{
						if (languageDefinition->mPreprocIdentifiers.count(id) != 0)
							token_color = PaletteIndex::PreprocIdentifier;
					}
				}
...
void TextEditor::ColorizeInternal()
{
	...
	if (mCheckComments)
	{
		...
		while (currentLine < endLine || currentIndex < endIndex)
		{
			...
			if (currentIndex == 0 && !concatenate)
			{
				...
				languageDefinition = withinLua ? luaLanguageDefinition : mLanguageDefinition;
			}
			...
			if (!line.empty())
			{
				...
				else
				{
					...
					else
					{
						...
						auto& singleStartStr = languageDefinition->mSingleLineComment;
						...
						else if (!withinSingleLineComment && currentIndex + sEndLua.size() <= line.size() &&
							ColorizerEquals(sEndLua.begin(), sEndLua.end(), from, from + sEndLua.size(), pred))
						{
							mSyntaxType[currentLine] = 0;
							withinLua = false;
						}
						else if (!withinSingleLineComment && currentIndex > 0 && line[currentIndex-1].mChar!='d' && currentIndex + sLua.size() <= line.size() &&
							ColorizerEquals(sLua.begin(), sLua.end(), from, from + sLua.size(), pred))
						{
							mSyntaxType[currentLine] = 0;
							withinLua = true;
						}
		...
		for (int currentLine = 0; currentLine < endLine; currentLine++) {
			if(mSyntaxType[currentLine] != prevSyntaxType[currentLine]) {
				forceRecolor.push_back(std::make_pair(currentLine, currentLine+1));
	...
	{
		const int increment = (mLanguageDefinition->mTokenize == nullptr) ? 1500 : 10000; // 10:10000
